...
const data = {
  Settings: {
    Yetkiler: ["817355812887330817"],
    Erkek: ["816265923589570570"],
    Kiz: ["816265924231036999"],
    KayitsizRolleri: ["816265921424654387"]
  },

}
let kayıtYetkili = '817355812887330817' //Yetkili
let erkekRole = '816265923589570570' //ERKEK ROL 1
let erkekRole2 = '816265925967085629'//ERKEK ROL 2
let kizRole = '816265924231036999'//SPANKERIN KARISI ROLÜ 1
let kizRole2 = '816265926869647382'//SPANKERIN KARISI ROLÜ 2
let kayıtsızRole = '816265921424654387' //Alınacak
let tag = '<>' //İsmin önüne gelecek tag
let ikinciTag = '•'// • BUNUN GİBİ TAG (•)        let ikinciTag = '•'
const erkekrol = message.guild.roles.cache.find(r => r.id === '816265923589570570') //erkekrol ismini değişmeyin
const erkekrol2 = message.guild.roles.cache.find(r => r.id === '816265925967085629') //erkekrol ismini değişmeyin
const kadınrol = message.guild.roles.cache.find(r => r.id === '816265924231036999') //kadınrol isimini değişme
const kadınrol2 = message.guild.roles.cache.find(r => r.id === '816265926869647382') //kadınrol isimini değişme
    //yetkili rol id
if(!["817355812887330817"].some(role => message.member.roles.cache.get(role)) && (!message.member.hasPermission("ADMINISTRATOR"))) 
...
if(!member.user.username.includes("TAGINIZ") && !member.roles.cache.has("816997227213226034") && !member.roles.cache.has("816265927305199636")) {
...
return;
	  ...
	  let Erkek = ("816265923589570570")
    let Kadin = ("816265924231036999")
      data.Settings[seçim].forEach(async rol => {
        
        if(seçim === "Erkek"){
          member.roles.add("816265923589570570") //erkek1
          member.roles.add("816265925967085629")//erkek2
          member.roles.remove("816265921424654387")//kayıtsız 
...
let alldata = rdb.fetch(`yetkili.${message.author.id}.toplam`)
  ...
  tag: tag
          ...
          member.roles.add("816265924231036999") //karı 1
          member.roles.add("816265926869647382") //karı 2
          member.roles.remove("816265921424654387")
